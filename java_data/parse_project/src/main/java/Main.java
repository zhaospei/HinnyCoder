...
public class Main {
    private static class Visitor extends ASTVisitor {
        ...
        // 3 types below are children of AbstractTypeDeclaration
        @Override
    ...
    public static class MethodSignatureVisitor2 extends ASTVisitor {
        ...
        // 3 types below are children class of Annotation 
        // (3 kinds of annotation according to jdt definition)
        @Override
    ...
    private static JsonArray parseType(CompilationUnit cu, String relativePath) {
        ...
        if (numType == 0) {
            ...
            for (int i = 0; i < numType; i++) {
                ...
                typeInfo.addProperty("name", type.getName().getIdentifier());
                String modifiers = "";
        ...
        if (files.size() > 0) {
            for (String file : files) {
                try {
                    ...
                    JsonArray typeInfos = parseType(cu, relativePath);
                    if (typeInfos != null) {
